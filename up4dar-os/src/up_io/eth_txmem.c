...
void eth_txmem_flush_q (void)
{
	...
	if (eth_tx_active != 0)
	{
		if ((AVR32_MACB.tsr & 0x01) != 0) // UBR used bit read, previous tx_buffer_q
								// is fully processed
		{
			AVR32_MACB.tsr = 0x01; // erase UBR bit
			eth_tx_active = 0;
			eth_txmem_cleanup(); // free used memory
		}
		else
		{
			return;  // don't write anything to tx_buffer_q yet, MACB is working
		}
	}
